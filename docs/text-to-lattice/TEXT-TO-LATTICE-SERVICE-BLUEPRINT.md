...
| BP-LINE-03 | Line of trust and network | Browser, same-origin API, and external provider | After explicit confirmation, the browser sends exactly text, requested_mode, and schema_version 1 to same-origin /api/lattice; the Worker alone holds HF_TOKEN and calls the fixed Hugging Face router and Nscale-served generator and DeepInfra-served verifier. |
...
| **BP-05 · Analyze with fixed generator** | Receives progress semantics only; cannot select a provider, URL, model, prompt, header, or credential. | The interface continues to describe external processing, not browser-local inference. | The Worker creates bounded prompts, segments protected source spans, and uses the fixed Qwen/Qwen3-4B-Instruct-2507:nscale generator for atomization, classification, and planning; remote clarification remains disabled with questions required to be empty. | Only https://router.huggingface.co/v1/chat/completions is eligible; each provider call has a 120-second timeout and bounded structured output. | Malformed, oversized, hostile, or unavailable upstream output fails closed or receives only bounded same-model schema correction within the overall pipeline budget. | Server-created prompts containing submitted text cross to Hugging Face, Nscale, and DeepInfra; the browser never receives raw provider bodies. |
...
- **Network boundary:** Server-created prompts containing submitted text cross to Hugging Face, Nscale, and DeepInfra; the browser never receives raw provider bodies.
