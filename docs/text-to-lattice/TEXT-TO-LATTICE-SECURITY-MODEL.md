...
- **Follow-up:** After the repaired deployment, retain one supported-browser non-error terminal conversion plus the repeated acquisition, release, and two-origin privacy trace. Repeat the functional lifecycle after any consequential widget, key, hostname, action, lease protocol, origin, provider, telemetry, runtime, or request-contract change. Observe a bodyless real renewal when an ordinary session naturally reaches the renewal interval; do not create a special wait or harness solely to manufacture it.
- **Rollback condition:** Set GATE-06 to open-release-blocker and return the overall release and public client immediately to the held documentation-only artifact if the repaired runtime fails before a non-error terminal conversion result, acquisition fails, release fails, any observed real renewal attempt fails, or any trace contains source text, clarification text, candidate text, verifier findings, output text, or an undocumented content-bearing request.
