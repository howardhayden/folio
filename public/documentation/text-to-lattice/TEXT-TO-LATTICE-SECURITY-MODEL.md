...
| GATE-02 | Production origin and secret boundary | Open Release Blocker | High | The deployed hah.dev and verify.hah.dev origins, lease Worker route, response policies, bindings, and independent encrypted secrets match the documented protocol, and every live noninteractive or intentionally invalid-token probe fails closed. | During the 2026-09-09 review, the live lease path returned HTTP 404, verify.hah.dev did not resolve, and the reviewed hah.dev response did not carry Content-Security-Policy or Permissions-Policy. Source contracts and deployment automation cannot substitute for those absent public boundaries. | Deploy the lease service and verification frame, configure independent encrypted secrets and exact routes, add the required response policies, inspect live headers, origins, routes, bindings, and secret names without disclosing values, then pass the checked-in method, cross-origin, cookie, missing-attestation, and intentionally invalid-attestation probes. A genuine widget token or successful lease grant is not preactivation evidence for this gate. |
...
| GATE-06 | Production lifecycle and privacy trace | Post Deployment Verification | High | The activated canonical page completes one real production-widget acquisition, renewal no sooner than the five-minute server minimum, and release with the documented 200, 200, and 204 statuses; its sanitized network trace shows no source, clarification, candidate, verifier finding, or output in lease, attestation, model-asset, error, or telemetry traffic. | Local lifecycle, data-flow, and request policies are implemented and tested, and GATE-02 binds live fail-closed probes that need no genuine challenge token. Those controls do not prove the real Turnstile key pair, activated canonical-page lifecycle, final browser behavior, or third-party request surface. | In the first activation session, use only the canonical public page in the supported browser engines to finish the production Turnstile challenge, record a 200 acquisition, wait through at least the five-minute server renewal minimum, record a 200 renewal and 204 release, and review a sanitized trace from both origins. Do not create a public or operator bypass harness to manufacture this evidence while the client is held. |
...
- **Safeguards:** the held source imports no interactive project surface; the held artifact validator rejects lease, verification, model, and runtime execution surfaces; documentation and the canonical project page remain available without inference; the live service verifier exercises public fail-closed behavior without a valid Turnstile token or lease credential
- **Follow-up:** Provision the exact two-origin service boundary, run the live configuration inspection and noninteractive or invalid-token probes, and change this status only from recorded evidence. Keep the real canonical-page lifecycle in GATE-06 rather than creating a bypass harness for the held client.
...
- **Rationale:** A broken real-key-pair lifecycle would make the newly activated interaction unusable or strand capacity, and source-text disclosure would be a high-consequence privacy failure. The held artifact intentionally exposes no client, so genuine official-page evidence can exist only after activation. Immediate verification with rapid held rollback has greater reliability, privacy, and lifecycle value than adding a bespoke preactivation harness and its new attack surface.
- **Evidence:** acquisition, delayed-renewal, release, and teardown source contracts; bodyless lease request contract; closed attestation message schema; browser-local model execution and output protection tests; source-level prohibition on content telemetry
...
- **Follow-up:** Complete and review the lifecycle and privacy trace in the first activation session, before declaring the release operationally complete, and repeat it after any widget, key, hostname, action, lease protocol, origin, provider, telemetry, runtime, or request-contract change.
- **Rollback condition:** Set GATE-06 to open-release-blocker and return the overall release and public client immediately to the held documentation-only artifact if real acquisition fails, renewal after the five-minute minimum fails, release fails, or any trace contains source text, clarification text, candidate text, verifier findings, output text, or an undocumented content-bearing request.
