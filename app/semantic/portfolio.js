...
import {
  ...
  SITE_ORIGIN, SITE_REPOSITORY, SITE_SOURCE_TERMS_URL,
  toolsSocial,
...
export const namespaceTerms = Object.freeze([
  ...
  ["sourceCodeTerms", "source-code terms", "The proprietary terms governing Owner-controlled site source rather than authored portfolio content.", "Property"],
...
function projectJsonLdNode(project) {
  ...
  return {
    ...
    [namespaceGraphId("capability")]: record.capabilities.items,
    [namespaceGraphId("technology")]: record.technologies.items,
    [namespaceGraphId("emphasis")]: record.emphasis,
    [namespaceGraphId("evidence")]: record.evidence.map(({ url }) => url),
    [namespaceGraphId("limitation")]: record.limitations.items,
    [namespaceGraphId("documentation")]: record.documentation,
    ...(documentation.length ? { hasPart: documentation.map(({ htmlUrl }) => ({ "@id": htmlUrl })) } : {}),
    [namespaceGraphId("publicationPrecision")]: record.publication.precision,
    [namespaceGraphId("statusAsOf")]: record.status.asOf,
    [namespaceGraphId("interactiveRelease")]: record.interactiveRelease,
    [namespaceGraphId("provenance")]: record.provenance,
    [namespaceGraphId("projectLicenseDocumented")]: record.license.documented,
    [namespaceGraphId("inheritedRequirement")]: record.inheritedRequirements.map((id) => ({ "@id": id })),
    [namespaceGraphId("relationship")]: record.relationships.map((relationship) => ({
      "@type": namespaceGraphId("ProjectRelationship"),
      [namespaceGraphId("predicate")]: relationship.predicate,
      [namespaceGraphId("target")]: { "@id": relationship.target },
      [namespaceGraphId("scope")]: relationship.scope,
    })),
  };
}

const graphContext = "https://schema.org";
...
const experienceNodes = resumeManifest.experience.map((entry) => ({
  ...
  [namespaceGraphId("periodLabel")]: entry.period,
  [namespaceGraphId("heldBy")]: { "@id": person.id },
  [namespaceGraphId("organization")]: { "@type": "Organization", name: entry.organization.name },
  [namespaceGraphId("engagementKind")]: entry.engagementKind,
  [namespaceGraphId("ongoing")]: entry.ongoing,
  [namespaceGraphId("detail")]: entry.details,
}));

const detailNodes = resumeManifest.expandedDetails.map((detail) => ({
  "@id": `${detail.canonicalUrl}#record`,
  "@type": ["CreativeWork", namespaceGraphId("ResumeRecord")],
  name: detail.title,
  ...
  [namespaceGraphId("detail")]: [...detail.details, ...detail.courses],
}));
...
const thirdPartyDocuments = Object.freeze([
  ...
  ["Llama 3.1 Community License", "/LICENSES/Llama-3.1-Community-License.txt", "text/plain", "Agreement snapshot supplied for the active Llama 3.1 verifier."],
  ["Llama 3.1 Acceptable Use Policy", "/LICENSES/Llama-3.1-Acceptable-Use-Policy.md", "text/markdown", "Acceptable-use policy snapshot supplied for the active Llama 3.1 verifier."],
  ["Historical Llama 3.2 Community License", "/LICENSES/Llama-3.2-Community-License.txt", "text/plain", "Agreement snapshot retained for the inactive historical Llama 3.2 verifier."],
  ["Historical Llama 3.2 Acceptable Use Policy", "/LICENSES/Llama-3.2-Acceptable-Use-Policy.md", "text/markdown", "Acceptable-use policy snapshot retained for the inactive historical Llama 3.2 verifier."],
  ...
  "@graph": [
    {
      ...
      license: SITE_CONTENT_TERMS_URL, [namespaceGraphId("sourceCodeTerms")]: SITE_SOURCE_TERMS_URL,
    },
    {
    ...
    pageNode("/third-party-notices/", "Legal and third-party notices", "Current Owner terms, historical license records, and third-party attribution for hah.dev and Text to Lattice.", "WebPage", thirdPartyDocumentsId),
    ...
    ...namespaceNodes,
    {
      ...
      "@type": "ItemList",
      name: "hah.dev legal notices and license documents",
    ...
    { "@id": projectsListId, "@type": "ItemList", name: "Projects", itemListElement: projects.map((project, index) => ({ "@type": "ListItem", position: index + 1, item: { "@id": projectGraphId(project) } })) },
    {
      ...
      [namespaceGraphId("interactiveRelease")]: applicationReleaseStatus,
      [namespaceGraphId("publicationMode")]: applicationPublicationMode,
      ...
      [namespaceGraphId("usagePolicy")]: {
        ...
        globalUtcDayLimit: 30,
        ordinaryBrowserCookieJarUtcDayLimit: 3,
        quotaIdentity: "One opaque HttpOnly cookie scoped to /api/lattice until the next UTC-day boundary; no IP or browser fingerprint.",
      ...
      [namespaceGraphId("runtime")]: textToLatticeContract.implementation.runtime,
    },
    ...experienceNodes,
    ...detailNodes,
    ...
    },
    {
      ...
      itemListElement: shelfManifest.items.map((item, index) => ({
        "@type": "ListItem", position: index + 1,
        item: {
          "@type": "CreativeWork", name: item.title, ...shelfDate(item.date), [namespaceGraphId("displayDate")]: item.date,
          inLanguage: item.languages, publisher: item.publishers.map((name) => ({ "@type": "Organization", name })),
          author: item.authors.map((name) => ({ "@type": "Person", name })), [namespaceGraphId("collection")]: item.collections,
        },
      })),
    },
  ],
...
function usagePolicyMarkdown() {
  ...
  return [
    "### Usage policy",
    "",
    `- Endpoint: ${absoluteUrl("/api/lattice")} with same-origin POST only; no query, redirect, or provider origin; the browser may attach only the API-scoped opaque HttpOnly quota cookie`,
    ...
    "- Daily admission: at most 30 accepted transformations globally per UTC day and 3 per ordinary persistent browser cookie jar; no IP or browser fingerprint is used as a second quota identity",
...
export function renderLlmsTxt() {
  ...
  return `# hah.dev\n\n> ${person.headline}\n\nAuthoritative public portfolio map. ${releaseBoundary} After explicit confirmation, Text to Lattice first sends a content-free bodyless cookie-setup POST to the same-origin /api/lattice path and, after an accepted 204, exactly one content-bearing POST with {text, requested_mode, schema_version: 1}. Only the second request contains submitted text or can initiate provider processing. The browser may attach one API-scoped opaque HttpOnly quota cookie; the service accepts at most 30 transformations globally per UTC day and 3 per ordinary persistent cookie jar, without IP identity or browser fingerprinting. The server uses a fixed Nscale-served Qwen generator and DeepInfra-served Llama verifier through Hugging Face Inference Providers. hah.dev does not incorporate submitted text or results into these public files or retain them in application storage, raw-content logs, caches, queues, or analytics; the browser does not retry automatically, and the server does not fall back to another provider or model. Submitted content leaves hah.dev and is processed under external-provider policies.\n\n## Primary pages\n\n- [About](${SITE_ORIGIN}/)\n- [Resume](${absoluteUrl("/resume/")})\n- [Projects](${absoluteUrl("/projects/")})\n- [Requirements](${absoluteUrl("/requirements/")})\n- [Semantic vocabulary](${absoluteUrl("/ns/")})\n- [Legal and third-party notices](${absoluteUrl("/third-party-notices/")})\n- [Tools](${absoluteUrl("/tools/")})\n- [Shelf](${absoluteUrl("/shelf/")})\n\n## Lattice documentation\n\n${documentation}\n\n### Text to Lattice release evidence\n\n${releaseEvidence}\n\n## Structured records\n\n- [Project manifest](${absoluteUrl("/projects.json")})\n- [Resume manifest](${absoluteUrl("/resume.json")})\n- [Tools manifest](${absoluteUrl("/tools.json")})\n- [Shelf manifest](${absoluteUrl("/shelf.json")})\n- [Knowledge graph](${absoluteUrl("/knowledge-graph.jsonld")})\n- [Project schema, current v2](${absoluteUrl("/schemas/projects-v2.schema.json")})\n- [Project schema, archived v1](${absoluteUrl("/schemas/projects-v1.schema.json")})\n\n## Canonical text\n\n- [About Markdown](${absoluteUrl("/content/about.md")})\n- [Resume Markdown](${absoluteUrl("/content/resume.md")})\n- [Projects Markdown](${absoluteUrl("/content/projects.md")})\n- [Tools Markdown](${absoluteUrl("/content/tools.md")})\n- [Shelf Markdown](${absoluteUrl("/content/shelf.md")})\n- [Complete authoritative text](${absoluteUrl("/llms-full.txt")})\n\n## Provenance and terms\n\n- [Portfolio repository](${SITE_REPOSITORY})\n- Site content version: ${SITE_CONTENT_VERSION}\n- Last updated: ${SITE_CONTENT_UPDATED}\n- [Current source terms](${SITE_SOURCE_TERMS_URL})\n- [Authored portfolio-content terms](${SITE_CONTENT_TERMS_URL})\n- [Legal, historical, and third-party notices](${absoluteUrl("/third-party-notices/")})\n`;
}
...
export function renderLlmsFull() {
  ...
  return `# hah.dev — complete authoritative portfolio text\n\nAs of: ${SITE_CONTENT_UPDATED}\nVersion: ${SITE_CONTENT_VERSION}\nRepository: ${SITE_REPOSITORY}\nCurrent site source terms: ${SITE_SOURCE_TERMS_URL}\nAuthored portfolio-content terms: ${SITE_CONTENT_TERMS_URL}\n\n## Representation boundary\n\nThis file contains owner-authored public portfolio information. Only after explicit confirmation, Text to Lattice first sends a content-free bodyless cookie-setup POST to the same-origin /api/lattice path and, after an accepted 204, exactly one content-bearing POST with {text, requested_mode, schema_version: 1}. Only the second request contains submitted text or can initiate provider processing. The browser may attach one API-scoped opaque HttpOnly quota cookie; admission is limited to 30 transformations globally per UTC day and 3 per ordinary persistent cookie jar, without IP identity or browser fingerprinting. Its server uses a fixed Nscale-served Qwen generator and DeepInfra-served Llama verifier through the Hugging Face router. hah.dev does not incorporate source, prompts, candidates, findings, or results here or keep them in application storage, raw-content logs, caches, queues, or analytics. The browser does not retry automatically, and the server does not fall back to another provider or model. External providers process submitted content under their own policies; do not submit restricted information.\n\n## Requirements and practice standards\n\n${requirements}\n\n## Semantic vocabulary\n\n${vocabulary}\n\n---\n\n${renderAboutMarkdown()}\n\n---\n\n${renderResumeMarkdown()}\n\n---\n\n${renderProjectsMarkdown()}\n\n---\n\n${renderToolsMarkdown()}\n\n---\n\n${renderShelfMarkdown()}`;
}
