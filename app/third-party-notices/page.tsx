      ...
      <main className="container mt-4 page-view semantic-page" data-page-view="third-party-notices">
        <article>
          ...
          <p>{String(interactiveRelease) === "held"
            ...
            : "The enabled capability submits only after the visitor explicitly chooses Process with external service."} The browser sends one same-origin <code>POST /api/lattice</code> request whose JSON body contains exactly <code>{"{text, requested_mode, schema_version: 1}"}</code>. The server uses the fixed Qwen generator and Llama verifier through Hugging Face Inference Providers and Featherless AI.</p>
