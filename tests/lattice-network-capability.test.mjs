...
test("capabilityFetch fixes an exact browser-owned origin and strips ambient request context", async () => {
  let observed;
...
test("text existence, typing, and paste do not invoke transport; explicit request invokes one same-origin POST", async () => {
  let input = "";
  let calls = 0;
  input = "typed";
  input += " pasted";
...
test("remote lifecycle exposes validating, submitting, processing, and success", async () => {
  ...
  await requestRemoteLattice("Source", {
    ...
    fetchImpl: async () => jsonResponse({ result, schema_version: 1 }),
  });
...
test("remote errors retain a bounded machine-readable taxonomy", async () => {
  for (const [status, payload, expected, retryable] of [
    ...
    assert.equal(calls, 1, `${payload.error} must not be retried automatically`);
  }
});
